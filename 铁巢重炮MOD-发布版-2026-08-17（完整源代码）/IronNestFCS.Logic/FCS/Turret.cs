...
public class Turret {
    ...
    /// <summary>读取共享炮塔当前真实方向，并转换为火控系统使用的 0～360 度方位角。</summary>
    public bool TryGetCurrentDirection(out float angle) {
        angle = 0f;
        if (_turret == null) return false;
        var current = _turret.CurrentAngle;
        if (float.IsNaN(current) || float.IsInfinity(current)) return false;
        angle = Mathf.Repeat(-current, 360f);
        return true;
    }
