...
public class GunSystem {
    ...
    public IEnumerator LoadBullet(BulletType type) {
        ...
        while (BulletInChamber() != expectedShellId) {
            if (loadBulletButton != null
                ...
                && loadBulletButton.nextAllowedClickTime <= Time.time) {
    ...
    private IEnumerator SelectPowder(int count, int alreadyStaged) {
        ...
        while (StagedPowderCharges < count) {
            ...
            if (button.isActive
                && button.nextAllowedClickTime <= Time.time) {
                yield return FcsSceneInteractor.WaitAndClick(button);
                continue;
            }

            // 不在正常选药过程中主动 ResetAllUsedDispensers。上一发的原生复位可能
            // 仍在播放；反复重置会重新启动该过程，尤其在 X10 下会让刚选出的装药
            // 持续被清空。这里等待真实按钮恢复并按其冷却重试即可。
            if (Time.realtimeSinceStartup >= nextWaitLogAt) {
                MelonLogger.Warning(
                    ...
                    $"clickReady={button.nextAllowedClickTime <= Time.time}, " +
    ...
    public IEnumerator LoadPowder(int count) {
        ...
        var nextRammerAttemptAt = Time.time;
        var nextWaitLogAt = Time.realtimeSinceStartup + 10f;
        while (LoadedPowderCharges <= 0) {
            ...
            if (StagedPowderCharges <= 0) {
                ...
                nextRammerAttemptAt = Time.time;
                continue;
            }
            if (loadPowderButton != null
                && loadPowderButton.isActive
                && loadPowderButton.nextAllowedClickTime <= Time.time
                && Time.time >= nextRammerAttemptAt) {
                MelonLogger.Msg(
                ...
                nextRammerAttemptAt = Time.time + 3f;
            }
            else {
                yield return new WaitForSeconds(0.1f);
            }
    ...
    public IEnumerator WaitReadyForNextLoad() {
        if (_postFireRecoveryPending && _nextLoadReadyAt <= 0f) {
            // 兜底时限从发射时刻开始，并跟随游戏倍速。真实推弹按钮一旦恢复，
            // 应立即继续，不再等待炮身停止后才开始计算固定安全间隔。
            _nextLoadReadyAt = Time.time + PostFireRecoverySeconds;
        }
        while (_postFireRecoveryPending) {
            var shellRammerReady = IsChamberEmpty()
                                   && loadBulletButton != null
                                   && loadBulletButton.isActive
                                   && loadBulletButton.nextAllowedClickTime <= Time.time;
            var elevationStopped = gunController == null
                                   || gunController.elevationChangeVelocity == 0;
            var fallbackExpired = elevationStopped && Time.time >= _nextLoadReadyAt;
            if (shellRammerReady || fallbackExpired) break;
            yield return new WaitForSeconds(0.1f);
        }
        _postFireRecoveryPending = false;

        // 炮膛已清空但原生 pendingReload 尚未结束时，装药托盘仍可能保留上一发的
        // 短暂旧读数。真实推弹按钮恢复或兜底时限通过后清掉空膛残留，避免把旧 6 份
        // 误认成新装药，也避免再额外拖延推弹。
        // 膛内仍有遗留弹时不在这里处理，由遗留弹流程接管。
        if (gunController != null && IsChamberEmpty() && gunController.pendingReload) {
            // 真实发射后的调用到这里之前已经通过真实按钮状态或兜底时限；首次
            // 进局/热重载若没有 _postFireRecoveryPending，则也不应再额外空等。
