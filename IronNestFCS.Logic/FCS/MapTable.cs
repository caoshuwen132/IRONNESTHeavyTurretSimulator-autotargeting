...
public class MapTable {
    ...
    public ArtilleryTask? GetMarkTarget(int index) {
        if (!artilleries.TryGetValue(index, out var marker) || marker == null) {
            MelonLogger.Warning(
                $"[FCS] Manual target T{index} ignored: marker is not available");
            return null;
        }

        if (fireMission?.coordinateRoot == null) return null;
        if (marker.gameObject == null || !marker.gameObject.activeInHierarchy) {
            MelonLogger.Warning(
                $"[FCS] Manual target T{index} ignored: marker is not visible on the map");
            return null;
        }
        ...
        var mapPoint = new Vector2(missionPosition.x, missionPosition.y);
        if (!float.IsFinite(mapPoint.x) || !float.IsFinite(mapPoint.y)
            || !fireMission.coordinateRoot.rect.Contains(mapPoint)) {
            MelonLogger.Warning(
                $"[FCS] Manual target T{index} ignored: pointer is outside the map " +
                $"({missionPosition.x:F3},{missionPosition.y:F3})");
            return null;
        }
    ...
    private static void CopySourceMetadata(ArtilleryTask target, ArtilleryTask source) {
        ...
        target.isLocomotive = source.isLocomotive;
    ...
    public List<ArtilleryTask> GetAutoTargets(
        ...
        foreach (var location in GetAllFireMissionEntities()) {
            try {
                ...
                task.isLocomotive = IsLocomotive(location, entity, entityName);
    ...
    private bool TryRefreshAreaImpactPoint(
        ...
        var predictedTargetIds = new List<string> { task.sourceEntityId };
        ...
        foreach (var entityId in task.areaCoveredTargetIds) {
            ...
            try {
                ...
                predictedTargetIds.Add(entityId);
            }
            catch {
        ...
        var bestCoveredTargetIds = new List<string>();
        ...
        foreach (var candidate in BuildLiveAreaAimCandidates(
            ...
            var count = 0;
            var maxDistance = 0f;
            var coveredTargetIds = new List<string>();
            for (var index = 0; index < predictedTargets.Count; ++index) {
                var target = predictedTargets[index];
                var distance = Vector3.Distance(candidate, target);
                if (distance > radiusMission + 0.001f) continue;
                count++;
                if (index > 0) coveredTargetIds.Add(predictedTargetIds[index]);
            ...
            bestPoint = candidate;
            bestCoveredTargetIds = coveredTargetIds;
        }
        ...
        // HCHE 只有实时仍可覆盖至少三个目标时才保留范围方案。若目标移动、摧毁或
        // 消失导致覆盖不足，先把落点还原到主目标；调用方可在购弹前重新选择经济弹。
        if (task.bulletType == BulletType.HCHE && bestCount < 3) {
            bestCount = 1;
            bestPoint = predictedPrimary;
            bestCoveredTargetIds.Clear();
        }
        ...
        task.areaTargetCount = bestCount;
        task.areaCoveredTargetIds = bestCoveredTargetIds;
    ...
    private MotionTrack UpdateMotion(string entityId, Vector3 current) {
        // 实体位移发生在 Unity 的游戏时间轴上；使用缩放后的时间计算速度，
        // 避免 X3/X10 时把目标和炮位速度分别放大三倍/十倍。
        var now = Time.time;
    ...
    /// <summary>
    /// 列车任务的车头使用 Train Locomotive 图标，后续车厢使用 cara#～care# 一类 ID。
    /// 车站也含 Train 字样，必须明确排除，避免把固定设施误归入最高优先级。
    /// </summary>
    private static bool IsLocomotive(
        EntityLocation location, MapEntity entity, string entityName) {
        return ContainsLocomotiveMarker(entity.ID)
               || ContainsLocomotiveMarker(entityName)
               || ContainsLocomotiveMarker(entity.Icon)
               || ContainsLocomotiveMarker(GetIconSpriteName(location, entity));
    }

    private static bool ContainsLocomotiveMarker(string? value) {
        if (string.IsNullOrWhiteSpace(value)) return false;
        if (value.IndexOf("Train Station", StringComparison.OrdinalIgnoreCase) >= 0
            || value.IndexOf("TrainStation", StringComparison.OrdinalIgnoreCase) >= 0
            || value.IndexOf("Railway Station", StringComparison.OrdinalIgnoreCase) >= 0
            || value.IndexOf("RailwayStation", StringComparison.OrdinalIgnoreCase) >= 0
            || value.IndexOf("总站", StringComparison.Ordinal) >= 0
            || value.IndexOf("车站", StringComparison.Ordinal) >= 0) return false;

        var entitySeparator = value.IndexOf('#');
        if (entitySeparator == 4
            && value.StartsWith("car", StringComparison.OrdinalIgnoreCase)
            && char.IsLetterOrDigit(value[3])) return true;

        return value.IndexOf("Locomotive", StringComparison.OrdinalIgnoreCase) >= 0
               || value.IndexOf("Train", StringComparison.OrdinalIgnoreCase) >= 0
               || value.IndexOf("Railcar", StringComparison.OrdinalIgnoreCase) >= 0
               || value.IndexOf("Rail Car", StringComparison.OrdinalIgnoreCase) >= 0
               || value.IndexOf("Wagon", StringComparison.OrdinalIgnoreCase) >= 0
               || value.IndexOf("机车", StringComparison.Ordinal) >= 0
               || value.IndexOf("列车", StringComparison.Ordinal) >= 0
               || value.IndexOf("火车", StringComparison.Ordinal) >= 0
               || value.IndexOf("车厢", StringComparison.Ordinal) >= 0;
    }
    ...
    private static bool IsCommander(EntityLocation location, MapEntity entity, string entityName) {
        if (ContainsCommanderMarker(entity.ID)
            || ContainsCommanderMarker(entityName)
            || ContainsCommanderMarker(entity.Icon)
            || ContainsCommanderMarker(GetIconSpriteName(location, entity))) return true;
    ...
    /// <summary>
    /// 返回测绘台任务坐标根节点及位于纸面朝向玩家一侧的轻微深度偏移。
    /// 预览图形以任务坐标直接作为本地 XY，因此与 EntityLocation.LocalPosition、
    /// 范围弹优化落点和移动目标实时解算使用完全相同的坐标系。
    /// </summary>
    public bool TryGetPreviewSurface(out RectTransform root, out float localZ) {
        root = fireMission?.coordinateRoot!;
        localZ = 0f;
        ...
        try {
            ...
            var normal = root.TransformDirection(Vector3.forward);
            var towardCamera = camera.transform.position - root.position;
            // LineRenderer 默认的 View 对齐会让闭合圆环的一半切入倾斜桌面。
            // 预览改用 TransformZ 后仍保留少量抬升，避免纸面深度精度造成闪烁。
            localZ = Vector3.Dot(normal, towardCamera) >= 0f ? 0.025f : -0.025f;
        }
        catch {
            // 深度偏移失败只可能让线条与纸面共面，不影响任务坐标与火控解算。
            localZ = 0f;
        }
        return true;
    }
    ...
    /// <summary>
    /// 返回桌面锁定特效应跟随的位置。范围任务使用实际优化落点，使旋转准星与
    /// 爆炸范围圆心重合；普通自动任务跟随实体，手动 T1～T4 保持固定指针位置。
    /// </summary>
    public bool TryGetTargetDisplayPosition(ArtilleryTask task, out Vector3 position) {
        position = task.position;
        if (fireMission?.coordinateRoot == null || task.isSafeDischarge) return false;
        ...
        try {
            if (!entity.IsAlive || entity.Health <= 0
                                || (entity.State & MapEntityStates.Destroyed) != 0
                                || (entity.Role & EntityRoles.Enemy) == 0
                                || (entity.Role & EntityRoles.Ally) != 0) return false;
            position = task.usesAreaAimPoint
                ? task.position
                : ConvertEntityToMapPosition(location, entity);
            return IsFinite(position)
                   && fireMission.coordinateRoot.rect.Contains(
                       new Vector2(position.x, position.y));
        }
    ...
    /// <summary>
    /// 根据炮塔当前方位和本管炮当前仰角对应的射程，计算桌面任务坐标中的实际落点。
    /// 与 CreateTask 使用互逆的方向约定：0° 向上，90° 向右。
    /// </summary>
    public bool TryCreateCurrentImpactTask(
        float directionDeg,
        float distanceKm,
        BulletType bulletType,
        string displayName,
        out ArtilleryTask task,
        out string reason) {
        task = null!;
        reason = "";
        if (fireMission?.coordinateRoot == null || firingTurret?.turretBase == null) {
            reason = "map or firing origin is not bound";
            return false;
        }
        if (!float.IsFinite(directionDeg) || !float.IsFinite(distanceKm) || distanceKm <= 0f) {
            reason = $"invalid current solution {directionDeg:F1} deg/{distanceKm:F2} km";
            return false;
        }

        try {
            var origin2 = fireMission.ToLocalSpace(firingTurret.turretBase.position);
            var radians = directionDeg * Mathf.Deg2Rad;
            var direction = new Vector3(Mathf.Sin(radians), Mathf.Cos(radians), 0f);
            var position = new Vector3(origin2.x, origin2.y, 0f)
                           + direction * (distanceKm / DistanceScale);
            if (!fireMission.coordinateRoot.rect.Contains(new Vector2(position.x, position.y))) {
                reason = "current ballistic impact is outside the map";
                return false;
            }

            task = CreateTask(position);
            task.targetName = displayName;
            task.bulletType = bulletType;
            task.angel = Mathf.Repeat(directionDeg, 360f);
            task.distance = distanceKm;
