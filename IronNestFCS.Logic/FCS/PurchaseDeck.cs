...
public class PurchaseDeck {
    private const float PurchaseButtonReadyTimeout = 5f;
    ...
    public IEnumerator BuyShell(BulletType type, LeftRight leftRight) {
        ...
        var purchaseClicked = false;
        yield return FcsSceneInteractor.WaitAndClickWithTimeout(
            _buyButton,
            PurchaseButtonReadyTimeout,
            clicked => purchaseClicked = clicked);
        if (!purchaseClicked) {
            MelonLogger.Warning(
                $"[FCS] BuyShell: purchase controls stalled for {type}/{leftRight}; " +
                "returning to the bounded outer retry so the card can be staged again");
            yield break;
        }
    ...
    public IEnumerator BuyPowders() {
        ...
        if (!purchaseClicked) {
            MelonLogger.Warning(
                "[FCS] BuyPowders: purchase controls stalled; returning to the bounded " +
                "outer retry so the card can be staged again");
            yield break;
        }
        yield return new WaitForSeconds(2f);
    }
