...
public sealed class MapImpactPreview {
    ...
    private static readonly Color LeftColor = new(0.1f, 0.95f, 1f, 1f);
    private static readonly Color RightColor = new(1f, 0.58f, 0.08f, 1f);
    ...
    public void Update(bool enabled) {
        ...
        for (var index = 0; index < visuals.Count; ++index) {
            if (index >= entries.Count) {
                visuals[index].Root.SetActive(false);
                continue;
            }
    ...
    public void HideAll() {
        foreach (var visual in visuals) visual.Root.SetActive(false);
    }
    ...
    private sealed class PreviewVisual {
        ...
        public void Destroy() {
            ...
            DestroyLineMaterial(horizontal);
            DestroyLineMaterial(vertical);
